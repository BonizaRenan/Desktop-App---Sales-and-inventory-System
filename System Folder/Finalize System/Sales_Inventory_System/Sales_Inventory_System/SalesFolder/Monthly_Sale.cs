...
{
    public partial class Monthly_Sale : Form
    {
        public Monthly_Sale()
        {
            InitializeComponent();
        ...
        void loads()
        {
            try
            {
                String sql = "SELECT Date, Barcode, Name, Quantity, Price FROM record_outofstock WHERE month(Date) = month(now())";
                ...
                conn.Open();
                cmd.CommandText = "SELECT SUM(Price)AS Total FROM record_outofstock WHERE month(Date) = month(now())";
                cmd.ExecuteNonQuery();
            ...
            {
                MessageBox.Show("You Dont Have Record For This Month");
            }
        }

        private void Monthly_Sale_Load(object sender, EventArgs e)
        {
