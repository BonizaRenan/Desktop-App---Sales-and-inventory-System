...
{
    public partial class Daily_Sale : Form
    {
        public Daily_Sale()
        {
            InitializeComponent();
        ...
        void loads()
        {
            try
            {
                String sql = "SELECT Date, Barcode, Name, Quantity, Price FROM record_outofstock WHERE Date = curdate()";
                ...
                conn.Open();
                cmd.CommandText = "SELECT SUM(Price)AS Total FROM record_outofstock WHERE Date = curdate()";
                cmd.ExecuteNonQuery();
        ...
        private void Daily_Sale_Load(object sender, EventArgs e)
        {
